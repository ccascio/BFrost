 ...
 *
 * Auth: Application Passwords (basic-auth header). Built into WP core since 5.6 — no
 * plugin required. Generate one at Users → Profile → Application Passwords.
 */

export interface WpAuth {
  baseUrl: string; // e.g. https://my-site.example.com — no trailing slash, no /wp-json
  username: string;
  applicationPassword: string; // raw value, not URL-encoded
}
...
export interface WpPostInput {
  ...
  tags?: number[];
  slug?: string;
  date?: string; // ISO 8601, only used for status=future
}
...
function authHeader(auth: WpAuth): string {
  const raw = `${auth.username}:${auth.applicationPassword}`;
...
export async function createPost(auth: WpAuth, post: WpPostInput): Promise<WpPostResult> {
  ...
  const res = (await wpFetch(auth, '/posts', {
    method: 'POST',
