...
- [ ] **Reduce App.tsx complexity.** The Overview, Queue, and Workers tabs read entirely from worker-declared surfaces (continuing Workstream 1/3 work in `ROADMAP.md`). No worker-specific HTML/JSX in `web/src/App.tsx`. This is a hard prerequisite for the catalog (Workstream D) and the recipe presets above. _(Partial — cloud API keys form moved to provider worker manifests (2026-05-27); embedding config (`renderEmbeddingConfiguration`, 4 state vars, 2 functions, ~110 lines) moved to `web/src/workers/builtin/memory/dashboard.tsx` as `kind: 'embedding-config'` view (2026-05-29); `PlatformSettings.embeddingProvider` widened from `'local'|'openai'` to `string`. Remaining: `dashboard.lmStudio` named API field + TopBar pin/unload wiring (backend+frontend, needs design); `integrations.openaiConfigured || anthropicConfigured` hardcoded pill in System tab (small).)_
