...
- [x] **Plain-language item summaries.** `WorkerManifest.summarizeForAssistant?: (item) => string` is now declared; `core.items.query` calls it when available and falls back to the generic field-dump format. _(Done 2026-05-23 — `types.ts` + `tools.ts` in items-query)_
...
- [x] **In-app guided help.** A `HelpTip` component renders a "?" button that toggles a plain-text popover. Added to the "Installed worker status" and "Runtime readiness" panel headings. Manifest-level `help.md` integration deferred to a future pass. _(Done 2026-05-23 — `HelpTip` component in App.tsx + CSS)_
