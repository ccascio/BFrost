...
- [x] Backups + restore: implement the guarded SQLite restore/import tooling promised in the older roadmap; worker-owned tables included automatically. _(Done 2026-05-28 — `applyPendingRestoreIfAny` now opens the backup read-only, runs `PRAGMA integrity_check`, and aborts with an audit event if the result is not `ok`; successful restores emit a `backup_restored` event; 4 new restore-path tests cover the marker file, no-op, valid swap with `.pre-restore` safety copy, and the corrupted-backup exit criterion.)_
